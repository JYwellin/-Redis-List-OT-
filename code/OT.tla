...
POS == 1..Len(LIST)
LEN == 1..Len(LIST)
Maxnum(set) == CHOOSE i \in set:\A j \in set:i>=j \* max number 
...
OP_1_set == [type: {"set"}, pos: POS, ch: CH, pr:PR] 
OP_2 == [type: {"del_r","ins_r"}, pos: POS, pr:PR, str:STR, len:LEN] 
OP_2_ins == [type: {"ins_r"}, pos: POS, pr:PR, str:STR] 
...
Xform_insr_set(ins,set) == ins
Xform_set_insr(set,ins) ==  
    IF set.pos >= ins.pos
    THEN  [set EXCEPT !.pos = @ +Len(ins.str)]
    ELSE set
Xform_delm_set(del,set) == del
Xform_set_delm(set,del) ==  
    CASE set.pos < del.ints[1][1] ->set
    [] \E i \in 1..Len(del.ints):del.ints[i][1] <= set.pos /\ set.pos < del.ints[i][1] + del.ints[i][2] -> NOP
    [] \E i \in 1..Len(del.ints)-1:set.pos >= del.ints[i][1] +  del.ints[i][2]/\ set.pos < del.ints[i+1][1] 
            -> [set EXCEPT !.pos = @ - dellen(del,@,0,Len(del.ints))]
    [] set.pos >= del.ints[Len(del.ints)][1] + del.ints[Len(del.ints)][2] 
             ->[set EXCEPT !.pos = @ - dellen(del,@,0,Len(del.ints))]
...
Xform(lop, rop) == \* the left operation is transformed against the right operation
    ...
    []  lop.type = "ins_r" /\ rop.type = "set" -> Xform_insr_set(lop, rop)  
    []  lop.type = "set" /\ rop.type = "ins_r" -> Xform_set_insr(lop, rop)
    []  lop.type = "del_m" /\ rop.type = "set" -> Xform_delm_set(lop, rop)  
    []  lop.type = "set" /\ rop.type = "del_m" -> Xform_set_delm(lop, rop)
...
correctness_1 (list) == \* CP1 correctness of the first kind of functions
    \A op1,op2 \in OP_1:
         \/ op1.pr = op2.pr
         \/ apply(apply(list,op1),Xform(op2, op1)) = apply(apply(list,op2),Xform(op1, op2))  


correctness_2 (list) == \* CP1 correctness of the second kind of functions
    \A op1,op2 \in OP_2:
         \/ op1.pr = op2.pr
         \/ apply(apply(list,op1),Xform(op2, op1)) = apply(apply(list,op2),Xform(op1, op2)) 
         
correctness_3 (list) == \* CP1 correctness of the third kind of functions
    \A op1,op2 \in OP_3, op3 \in OP_2_ins:
         /\ apply(apply(list,op1),Xform(op2, op1)) = apply(apply(list,op2),Xform(op1, op2)) 
         /\ apply(apply(list,op1),Xform(op3, op1)) = apply(apply(list,op3),Xform(op1, op3)) 
         
correctness_4 (list) == \* CP1 correctness of the fourth kind of functions
    \A op1 \in OP_1_set, op2 \in OP_2_ins,op3 \in OP_3:
         /\ apply(apply(list,op1),Xform(op2, op1)) = apply(apply(list,op2),Xform(op1, op2)) 
         /\ apply(apply(list,op1),Xform(op3, op1)) = apply(apply(list,op3),Xform(op1, op3)) 
         
correctness(list) == correctness_1(list) /\ correctness_2(list) /\ correctness_3(list) /\ correctness_4(list)
=============================================================================
\* Modification History
\* Last modified Tue May 15 20:34:16 CST 2018 by xhdn
